...
public class FilmeDAO {
    ...
    public ArrayList<Filme> listarMelhoresFilmes() {
        ConectDBPostgres cdb = new ConectDBPostgres();
        ArrayList<Filme> filmes = new ArrayList<>();
        try {
            Statement stmt = cdb.getConecao().createStatement();
            ResultSet rs = stmt.executeQuery("SELECT * FROM filme ORDER BY notamedia DESC NULLS LAST, notamedia != 0 DESC LIMIT 3");
            while (rs.next()) {
                Filme f = new Filme(
                        rs.getInt("idfilme"),
                        rs.getString("nomefilme"),
